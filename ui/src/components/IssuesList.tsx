  ...
  return (
    <div className="relative w-48 sm:w-64 md:w-80">
      ...
      <Input
        ...
        placeholder={t("issuesList.search.placeholder", { defaultValue: "Search issues..." }) as string}
        className="pl-7 text-xs sm:text-sm"
        aria-label={t("issuesList.search.ariaLabel", { defaultValue: "Search issues" }) as string}
        ...
        <div className="min-w-0 flex-1 space-y-2">
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm">
            <span className="font-medium text-foreground">
              {summary.doneCount}/{summary.totalCount} {t("issuesList.progress.done", { defaultValue: "done" })}
            </span>
            <span className="text-muted-foreground">
              {summary.inProgressCount} {t("issuesList.progress.inProgress", { defaultValue: "in progress" })}
            </span>
            <span className="text-muted-foreground">
              {summary.blockedCount} {t("issuesList.progress.blocked", { defaultValue: "blocked" })}
            </span>
            {showCostSummary && (
              <>
                <span
                  className="text-muted-foreground tabular-nums"
                  title={`${costSummary.runCount.toLocaleString()} ${t("issuesList.progress.runCount", { count: costSummary.runCount, defaultValue: costSummary.runCount === 1 ? "run" : "runs" })} across ${costSummary.issueCount} ${t("issuesList.progress.subIssueCount", { count: costSummary.issueCount, defaultValue: costSummary.issueCount === 1 ? "sub-issue" : "sub-issues" })}`}
                >
                  {formatTokens(totalTokens)} {t("issuesList.progress.tokens", { defaultValue: "tokens" })}
                </span>
                <span className="text-muted-foreground tabular-nums">
                  {formatDurationMs(costSummary.runtimeMs)} {t("issuesList.progress.runtime", { defaultValue: "runtime" })}
                </span>
              </>
            )}
          </div>
          <div
            role="progressbar"
            aria-label={t("issuesList.progress.progressBarAriaLabel", { defaultValue: "Sub-issues completion progress" }) as string}
            aria-valuemin={0}
          ...
          {target && targetIssue ? (
            <>
              <div className="text-xs font-medium text-muted-foreground">
                {target.kind === "next" ? t("issuesList.progress.nextUp", { defaultValue: "Next up" }) : t("issuesList.progress.waitingOnBlockers", { defaultValue: "Waiting on blockers" })}
              </div>
              <Link
            ...
            <div className="text-sm font-medium text-foreground">{t("issuesList.progress.noActiveSubIssues", { defaultValue: "No active sub-issues" })}</div>
          ) : summary.doneCount === summary.totalCount ? (
            <div className="text-sm font-medium text-foreground">{t("issuesList.progress.allSubIssuesDone", { defaultValue: "All sub-issues done" })}</div>
          ) : (
            <div className="text-sm font-medium text-foreground">{t("issuesList.progress.noActionableSubIssues", { defaultValue: "No actionable sub-issues" })}</div>
          )}
        </div>
      </div>
    </div>
...
export function IssuesList({
  ...
  const groupedContent = useMemo(() => {
    ...
    if (viewState.groupBy === "workspace") {
      ...
      return Object.keys(groups)
        ...
        .map((key) => ({
          key,
          label: key === "__no_workspace" ? t("issuesList.group.noWorkspace", { defaultValue: "No Workspace" }) as string : (workspaceNameMap.get(key) ?? key.slice(0, 8)),
          items: groups[key]!,
        }));
    }
    if (viewState.groupBy === "project") {
      ...
      return Object.keys(groups)
        ...
        .map((key) => ({
          key,
          label: key === "__no_project" ? t("issuesList.group.noProject", { defaultValue: "No Project" }) as string : (projectById.get(key)?.name ?? key.slice(0, 8)),
          items: groups[key]!,
        }));
    }
    if (viewState.groupBy === "parent") {
      ...
      return Object.keys(groups)
        ...
        .map((key) => ({
          key,
          label: key === "__no_parent" ? t("issuesList.group.noParent", { defaultValue: "No Parent" }) as string : (issueTitleMap.get(key) ?? key.slice(0, 8)),
          items: groups[key]!,
        }));
    }
    // assignee
    ...
    return Object.keys(groups).map((key) => ({
      ...
      label:
        key === "__unassigned"
          ? t("issuesList.group.unassigned", { defaultValue: "Unassigned" }) as string
  ...
  const createActionLabel = createIssueLabel
    ? t("issuesList.toolbar.createCustom", { label: createIssueLabel, defaultValue: `Create ${createIssueLabel}` })
    : t("issuesList.toolbar.createIssue", { defaultValue: "Create Issue" });
  const createButtonLabel = createIssueLabel
    ? t("issuesList.toolbar.newCustom", { label: createIssueLabel, defaultValue: `New ${createIssueLabel}` })
    : t("issuesList.toolbar.newIssue", { defaultValue: "New Issue" });
    ...
    <div ref={rootRef} className="space-y-4">
        ...
        <div className="flex items-center gap-0.5 sm:gap-1 shrink-0">
          ...
          <div className="flex items-center border border-border rounded-md overflow-hidden mr-1">
            <button
              ...
              title={t("issuesList.toolbar.listView", { defaultValue: "List view" }) as string}
            >
            ...
            <button
              ...
              title={t("issuesList.toolbar.boardView", { defaultValue: "Board view" }) as string}
            >
          ...
          {viewState.viewMode === "list" && (
            <Button
              ...
              title={viewState.nestingEnabled ? t("issuesList.toolbar.disableNesting", { defaultValue: "Disable parent-child nesting" }) as string : t("issuesList.toolbar.enableNesting", { defaultValue: "Enable parent-child nesting" }) as string}
            >
          ...
          {viewState.viewMode === "board" && (
            <>
              <Button
                ...
                title={boardCompactCards ? t("issuesList.board.useComfortableCards", { defaultValue: "Use comfortable cards" }) as string : t("issuesList.board.useCompactCards", { defaultValue: "Use compact cards" }) as string}
              >
              ...
              <Button
                ...
                title={boardCollapsedStatuses.length > 0 ? t("issuesList.board.expandColdLanes", { defaultValue: "Expand cold lanes" }) as string : t("issuesList.board.collapseColdLanes", { defaultValue: "Collapse cold lanes" }) as string}
              >
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button
                    ...
                    title={t("issuesList.board.cardsPerColumn", { defaultValue: "Cards per column" }) as string}
                  >
                ...
                <PopoverContent align="end" className="w-40 p-0">
                  <div className="p-2 space-y-0.5">
                    {KANBAN_COLUMN_PAGE_SIZE_OPTIONS.map((pageSize) => (
                      ...
                      >
                        <span>{pageSize} {t("issuesList.board.perColumn", { defaultValue: "per column" })}</span>
              ...
              <Button
                ...
                title={t("issuesList.board.resetDensity", { defaultValue: "Reset board density" }) as string}
              >
          ...
          <IssueColumnPicker
            ...
            title={t("issuesList.toolbar.columnPickerTitle", { defaultValue: "Choose which issue columns stay visible" }) as string}
            iconOnly
          />
          ...
          {viewState.viewMode === "list" && (
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="outline" size="icon" className="h-8 w-8 shrink-0" title={t("issuesList.sort.title", { defaultValue: "Sort" }) as string}>
          ...
          {viewState.viewMode === "list" && (
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="outline" size="icon" className="h-8 w-8 shrink-0" title={t("issuesList.group.title", { defaultValue: "Group" }) as string}>
              ...
              <PopoverContent align="end" className="w-44 p-0">
                <div className="p-2 space-y-0.5">
                  {([
                    ["status", t("issuesList.group.status", { defaultValue: "Status" })],
                    ["priority", t("issuesList.group.priority", { defaultValue: "Priority" })],
                    ["assignee", t("issuesList.group.assignee", { defaultValue: "Assignee" })],
                    ["project", t("issuesList.group.project", { defaultValue: "Project" })],
                    ["workspace", t("issuesList.group.workspace", { defaultValue: "Workspace" })],
                    ["parent", t("issuesList.group.parentIssue", { defaultValue: "Parent Issue" })],
                    ["none", t("issuesList.group.none", { defaultValue: "None" })],
      ...
      {!searchWithinLoadedIssues && normalizedIssueSearch.length > 0 && searchedIssues.length === ISSUE_SEARCH_RESULT_LIMIT && (
        <p className="text-xs text-muted-foreground">
          {t("issuesList.search.resultLimit", { limit: ISSUE_SEARCH_RESULT_LIMIT, defaultValue: `Showing up to ${ISSUE_SEARCH_RESULT_LIMIT} matches. Refine the search to narrow further.` })}
        </p>
      )}
      {boardColumnLimitReached && (
        <p className="text-xs text-muted-foreground">
          {t("issuesList.board.columnLimitReached", { limit: ISSUE_BOARD_COLUMN_RESULT_LIMIT, defaultValue: `Some board columns are showing up to ${ISSUE_BOARD_COLUMN_RESULT_LIMIT} issues. Refine filters or search to reveal the rest.` })}
        </p>
      )}
      {!isLoading && filtered.length === 0 && viewState.viewMode === "list" && (
        <EmptyState
          icon={CircleDot}
          message={t("issuesList.empty.noIssues", { defaultValue: "No issues match the current filters or search." }) as string}
          action={createActionLabel as string}
        ...
        <>
              ...
              <IssueGroupHeader
                ...
                trailing={(
                  <Button
                    ...
                    title={t("issuesList.group.newIssueIn", { label: group.label, defaultValue: `New issue in ${group.label}` }) as string}
                    aria-label={t("issuesList.group.newIssueIn", { label: group.label, defaultValue: `New issue in ${group.label}` }) as string}
                          ...
                          <>
                            {hasChildren && !isExpanded ? (
                              <span className="ml-1.5 text-xs text-muted-foreground">
                                ({totalDescendants} {t("issuesList.row.subTask", { count: totalDescendants, defaultValue: totalDescendants !== 1 ? "sub-tasks" : "sub-task" })})
                              </span>
                            ) : null}
                            {issueBadge ? (
                              issueBadge === "Paused" ? (
                                <span
                                  ...
                                  aria-label={t("issuesList.badge.paused", { defaultValue: "Paused" }) as string}
                                  title={t("issuesList.badge.paused", { defaultValue: "Paused" }) as string}
                                >
                            ...
                            {isSuccessfulRunHandoffRequired(issue) ? (
                              <span
                                ...
                                aria-label={t("issuesList.badge.needsNextStep", { defaultValue: "Needs next step" }) as string}
                                title={t("issuesList.badge.needsNextStepTitle", { defaultValue: "This issue needs a next step" }) as string}
                              >
                                ...
                                >
                                  ...
                                  >
                                    <input
                                      ...
                                      placeholder={t("issuesList.assignee.searchPlaceholder", { defaultValue: "Search assignees..." }) as string}
                                    ...
                                    <div className="max-h-48 overflow-y-auto overscroll-contain">
                                      ...
                                      >
                                        {t("issuesList.assignee.noAssignee", { defaultValue: "No assignee" })}
                                      </button>
                                      {currentUserId && (
                                        <button
                                          className={cn(
          ...
          {(remainingIssueRowCount > 0 || hasMoreIssues || isLoadingMoreIssues) && (
            <div className="py-2" data-testid="issues-load-more-sentinel">
              <p className="text-xs text-muted-foreground">
                {isLoadingMoreIssues
                  ? t("issuesList.pagination.loadingMore", { defaultValue: "Loading more issues..." })
                  : remainingIssueRowCount > 0
                    ? t("issuesList.pagination.rendering", { rendered: Math.min(renderedIssueRowLimit, filtered.length), total: filtered.length, defaultValue: `Rendering ${Math.min(renderedIssueRowLimit, filtered.length)} of ${filtered.length} issues` })
                    : t("issuesList.pagination.scrollToLoad", { defaultValue: "Scroll to load more issues" })}
              </p>
            </div>
          )}
        </>
